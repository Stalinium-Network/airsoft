...
async function getFractions(): Promise<Fraction[]> {
  try {
    ...
    if (!res.ok) {
      console.error('Failed to fetch fractions data:', res.status, res.statusText);
      return [];
    }
...
function FractionsSkeleton() {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      {[1, 2, 3, 4].map((i) => (
        <div key={i} className="bg-gray-800 rounded-lg overflow-hidden shadow-xl animate-pulse flex flex-col md:flex-row">
      ...
      <section id="fractions" className="py-20 px-4 w-screen bg-gray-900">
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-16">
            <h2 className="text-4xl font-bold mb-6">ZONE <span className="text-green-500">FRACTIONS</span></h2>
            <p className="text-xl text-gray-300 max-w-3xl mx-auto">
              Discover the various factions competing for power, resources, and artifacts in the Zone.
            </p>
          </div>
          <FractionsSkeleton />
        </div>
