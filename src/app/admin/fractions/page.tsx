...
export default function FractionsManagement() {
  ...
  const { token, logout, message, isError, setMessage, setIsError } = useAdminAuth()
  ...
  const deleteFraction = async (fractionId: string) => {
    ...
    if (!confirm('Are you sure you want to delete this fraction? This may affect games using this fraction.')) {
      return
    }
  ...
  const handleFractionUpdated = async (updatedFraction: Fraction) => {
    setFractions(prev => prev.map(f => f._id === updatedFraction._id ? updatedFraction : f))
  ...
  return (
    <AdminLayout>
      <div className="py-8 px-4 mx-auto max-w-7xl">
        ...
        {/* Messages */}
        {message && (
          <div className={`p-4 mb-8 rounded ${isError ? 'bg-red-900/50 border border-red-700' : 'bg-green-900/50 border border-green-700'}`}>
            {message}
          </div>
        )}
